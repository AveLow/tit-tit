...
abstract class CustomErrorsHandler extends AppComponent{
    ...
    abstract public function notFoundHandler(RequestInterface $request, ResponseInterface $response);
    // For php7.1 abstract public function notFoundHandler($request, $response): ResponseInterface;

    abstract public function notAllowedHandler(RequestInterface $request, ResponseInterface $response,array $allowedMethods);
    // For php7.1 abstract public function notAllowedHandler($request, $response, $exception): ResponseInterface;

    abstract public function phpErrorHandler(RequestInterface $request, ResponseInterface $response,\Throwable $exception);
